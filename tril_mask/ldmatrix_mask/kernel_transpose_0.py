...
Problems:
- I made an smem layout B but that's swizzle applied to the transposed layout which obviously isn't the same as swizzle applied to the original layout
- There's the transpose view which basically just swaps the first two dims and that works.
"""
...
class Kernel:
    ...
    def kernel(self, a: cute.Tensor, b: cute.Tensor, smem_layout: cute.ComposedLayout, smem_layout_b: cute.ComposedLayout, copy_a: cute.TiledCopy, copy_b: cute.TiledCopy):
        ...
        As_t = transpose_view(As)
        print('As_t.layout:', As_t.layout)
        print('My layout:', smem_layout_b)
        ...
        cute.autovec_copy(As_t, b_tiled)
