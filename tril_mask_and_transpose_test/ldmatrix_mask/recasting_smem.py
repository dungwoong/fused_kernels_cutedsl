...
class Kernel:
    ...
    def kernel(self, a: cute.Tensor, b: cute.Tensor, smem_layout: cute.ComposedLayout, smem_layout_b: cute.ComposedLayout, copy_a: cute.TiledCopy, copy_b: cute.TiledCopy):
        ...
        SS2 = type("SS2", (), dict())
        SS2.__annotations__['bar_ptr'] = cute.struct.MemRange[cutlass.Int64, 3*2]


        ss_type = cute.struct(SharedStorage)
        print('ALIGNOF', ss_type.__alignof__())
        smem_allocator_ = cutlass.utils.SmemAllocator()
        smem__ = smem_allocator_.allocate(ss_type)
        smem_bars = smem_allocator_.allocate(cute.struct(SS2))
