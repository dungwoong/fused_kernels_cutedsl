...
class FlashSM90:
    ...
    def mma(self, n_block_max: int, 
        ...
        while work_tile.is_valid_tile:
            m_block, head_idx, batch_idx = work_tile.tile_idx
            if cutlass.const_expr(self.intra_wg_overlap):
                kv_consumer_state, row_scale = self.first_half_block_overlap(mma_qk, tSrQ, tSrK, tOrP, pipeline_k, kv_consumer_state, softmax)
                O_should_accumulate = False
                for _ in cutlass.range(n_block_max-1, unroll=1):
                    kv_consumer_state, row_scale = self.mma_one_n_block_iwo(pipeline_k, pipeline_v, kv_consumer_state, mma_qk, tSrQ, tSrK, softmax, tOrP, acc_o, mma_pv, tOrVt, O_should_accumulate, False, row_scale)
                    O_should_accumulate = True
                kv_consumer_state = self.last_half_block_overlap(acc_o, tOrP, tOrVt, kv_consumer_state, pipeline_v, mma_pv, softmax, row_scale)
            if cutlass.const_expr(not self.intra_wg_overlap):
    ...
    def first_half_block_overlap(self, mma_qk: cute.TiledMma, tSrQ: cute.Tensor, tSrK: cute.Tensor, tOrP: cute.Tensor, pipeline_k: pipeline.PipelineAsync, kv_consumer_state: pipeline.PipelineState, softmax: Softmax):
        ...
        row_scale = softmax.online_softmax(p_acc, is_first=True) # no need to rescale on first iter
        ...
        return kv_consumer_state, row_scale
    ...
    def mma_one_n_block_iwo(self, 
        ...
        self.inter_wg_arrive() # TODO ping-pong isn't really relevant anymore
    ...
    @cute.jit
    def last_half_block_overlap(self, acc_o: cute.Tensor, tOrP: cute.Tensor, tOrVt: cute.Tensor, kv_consumer_state: pipeline.PipelineState, pipeline_v: pipeline.PipelineAsync, mma_pv: cute.TiledMma, softmax: Softmax, row_scale: cute.Tensor):
        softmax.rescale_O(acc_o, row_scale)
        pipeline_v.consumer_wait(kv_consumer_state, pipeline_v.consumer_try_wait(kv_consumer_state))
...
if __name__ == "__main__":
    ...
    fa = FlashSM90(qk_mn=(128, 128), num_stages=3, cluster_size_m=1, intra_wg_overlap=True, pingpong=False, mma_m_size=64)
